...
class CheckoutObserver implements ObserverInterface
{
	...
	/**
	 * Method called when event controller_action_predispatch_checkout_index_index is triggered
	 *
	 * Determines if a user is trying to checkout during a period when the store should be closed
	 *
    ...
    public function execute(Observer $observer)
    {
		...
		if($this->loadData($this->store_status))
		{
			...
			$this->managerInterface->addErrorMessage("The store is closed, so you cannot purchase any items");
		}
    }
